...
iptables -A anti-port-scan -p tcp --tcp-flags SYN,ACK,FIN,RST RST -m limit --limit 20/s -j RETURN
iptables -A anti-port-scan -j DROP

# block hosts that have more than 30 established connections
iptables -A INPUT -p tcp -m connlimit --dport 25565 --connlimit-above 30 -j REJECT --reject-with tcp-reset
